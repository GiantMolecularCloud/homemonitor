...
#############################################################################
    def load_data(self):
        ...
        # load data
        self.data = np.genfromtxt(os.path.join(self.plotdir,self.logname+'.csv'), delimiter=',', skip_header=1, dtype=('U19','<i8','<f8'), names=('date','co2','temp'))
...
#############################################################################
    ...
    def plot_hist(self):
        ...
        ax_t.hist(self.data['temp'][self.idx_3month:-1], bins=hist_temp1[1], histtype='stepfilled', color='midnightblue', label='last month')
        ax_co.hist(self.data['co2'][self.idx_3month:-1], bins=hist_co21[1],  histtype='stepfilled', color='midnightblue', orientation='horizontal', label='last month')
